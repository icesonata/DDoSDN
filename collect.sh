...
do
    ...
    ipsrc=$(awk -F "," '{out=""; for(i=2;i<=NF;i++){out=out" "$i}; print out}' data/flowentries.csv | awk -F " " '{split($11,d,"="); print d[2]","}')
    ipdst=$(awk -F "," '{out=""; for(i=2;i<=NF;i++){out=out" "$i}; print out}' data/flowentries.csv | awk -F " " '{split($12,d,"="); print d[2]","}')
    ...
    default_flow=$(sudo ovs-ofctl dump-flows s1 | tail -n 1)    # Get flow "action:CONTROLLER:<port_num>" sending unknown packet to the controller
    ...
    then
        ...
        for i in {1..6}
        do
            default_flow=$(sudo ovs-ofctl dump-flows s$i | tail -n 1)    # Get flow "action:CONTROLLER:<port_num>" sending unknown packet to the controller
            sudo ovs-ofctl del-flows s$i
            sudo ovs-ofctl add-flow s$i "$default_flow"
        done
    fi
    sleep 3
done
